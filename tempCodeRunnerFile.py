...
def get_image_url(page_url, image_resolution):
    ...
    if image_type == 0:
        page = requests.get(
            page_url, headers={"User-Agent": random.choice(USER_AGENTS)}
        )
        ...
        if image_resolution == 0:
            ...
            result.append(DOWNLOAD_URL.format(res=image_resolution, part=part, img=img))
    elif image_type == 2:
        page = requests.get(
            page_url, headers={"User-Agent": random.choice(USER_AGENTS)}
        )
        ...
        page = requests.get(
            page_url, headers={"User-Agent": random.choice(USER_AGENTS)}, timeout=10
...
def parse_arguments(args):
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        choices=[0, 16, 24, 32, 64, 128, 256, 512],
    )
...
def main():
    logo()
    search = "cat"  # Modify search term here
    url = f"https://www.flaticon.com/search?word={search}&license=selection&style=all-styles"
    arguments = parse_arguments([])
    print(f'\033[92mGetting images related to "{search}"\n\033[0m')
    images = get_image_url(url, arguments.resolution)
    print(f"\033[92mThe following image(s) will be downloaded:\033[0m")
    print("\n".join(images))
